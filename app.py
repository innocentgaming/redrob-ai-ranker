...
# FPDF Class for Executive Summary PDF Report
class ExecutiveSummaryPDF(FPDF):
    def header(self):
        self.set_fill_color(37, 99, 235) # Accent blue
        self.rect(0, 0, 210, 15, 'F')
        self.set_text_color(255, 255, 255)
        self.set_font("Arial", 'B', 10)
        self.set_y(5)
        self.cell(0, 5, "REDROB AI RECRUITER - CANDIDATE EXECUTIVE SUMMARY", 0, 1, 'C')
        self.set_y(20)
    ...
    def footer(self):
        ...
        self.cell(0, 10, f"Page {self.page_no()} | Generated by Redrob AI Recruiter Platform", 0, 0, 'C')

def generate_candidate_pdf(sc, xai):
    c = sc["candidate"]
    prof = c["profile"]
    ...
    pdf.cell(0, 10, prof["name"], 0, 1)
    
    pdf.set_font("Arial", 'I', 11)
    pdf.set_text_color(113, 113, 122)
    pdf.cell(0, 6, prof["headline"], 0, 1)
    pdf.ln(4)
    
    # Score block
    pdf.set_fill_color(244, 244, 245)
    pdf.rect(10, 40, 190, 22, 'F')
    
    pdf.set_xy(15, 43)
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(37, 99, 235)
    pdf.cell(80, 5, f"Hiring Probability: {xai['hiring_probability']:.1f}%", 0, 0)
    
    pdf.set_font("Arial", 'B', 11)
    pdf.set_text_color(9, 9, 11)
    pdf.cell(100, 5, f"Rating: {xai['rating'].upper()}", 0, 1, 'R')
    
    pdf.set_xy(15, 51)
    pdf.set_font("Arial", '', 9)
    pdf.set_text_color(82, 82, 91)
    pdf.cell(0, 5, f"Location: {prof['current_location']} | Experience: {prof['years_of_experience']} Years | Notice Period: {sigs['notice_period_days']} Days", 0, 1)
    pdf.set_xy(10, 68)
    ...
    pdf.multi_cell(0, 5, prof["summary"])
    pdf.ln(4)
    
    # Strengths & Gaps
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(9, 9, 11)
    pdf.cell(0, 6, "AI Recruiter Evaluation", 0, 1)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(2)
    
    pdf.set_font("Arial", 'B', 9.5)
    pdf.set_text_color(22, 163, 74)
    pdf.cell(0, 5, "Key Strengths:", 0, 1)
    pdf.set_font("Arial", '', 9)
    pdf.set_text_color(30, 30, 30)
    for s in xai["strengths"]:
        pdf.multi_cell(0, 4.5, f"- {s}")
    pdf.ln(2)
    
    pdf.set_font("Arial", 'B', 9.5)
    pdf.set_text_color(217, 119, 6)
    pdf.cell(0, 5, "Potential Gaps & Risks:", 0, 1)
    pdf.set_font("Arial", '', 9)
    pdf.set_text_color(30, 30, 30)
    if xai["weaknesses"]:
        for w in xai["weaknesses"]:
            pdf.multi_cell(0, 4.5, f"- {w}")
    else:
        pdf.cell(0, 4.5, "No major gaps or risks flagged.", 0, 1)
    pdf.ln(4)
    
    # Career History
    pdf.set_font("Arial", 'B', 12)
    pdf.set_text_color(9, 9, 11)
    pdf.cell(0, 6, "Career History", 0, 1)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(2)
    ...
    for job in c.get("career_history", []):
        pdf.set_font("Arial", 'B', 9.5)
        pdf.set_text_color(9, 9, 11)
        pdf.cell(120, 5, f"{job.get('title')} at {job.get('company')}", 0, 0)
        pdf.set_font("Arial", 'I', 9)
        pdf.set_text_color(113, 113, 122)
        pdf.cell(70, 5, f"{job.get('start_date')[:7]} to {job.get('end_date')[:7]}", 0, 1, 'R')
        
        pdf.set_font("Arial", '', 9)
        pdf.set_text_color(82, 82, 91)
        pdf.multi_cell(0, 4.5, job.get('description'))
        pdf.ln(2)
...
with st.sidebar:
    ...
    st.subheader("4. AI Co-Pilot Settings")
    gemini_key = st.text_input("Gemini API Key", type="password", help="Enter your Gemini API key to activate the AI Candidate Chat.")
    if gemini_key:
...
with tab_dash:
    ...
    # Talent Location Distribution Map
    st.markdown(f"""
    <div class="chart-wrap">
        <div class="chart-title">🗺️ Talent Location Distribution</div>
        <div class="chart-subtitle">Geographical distribution of candidates and their willingness to relocate</div>
    """, unsafe_allow_html=True)
    ...
    for sc in scored_candidates:
        loc = sc["candidate"]["profile"].get("current_location", "Unknown")
        relocate = sc["candidate"]["redrob_signals"].get("willing_to_relocate", False)
        map_data.append({
            "Location": loc,
            "Relocate": "Willing to Relocate" if relocate else "Not Willing to Relocate",
            "Score": sc["final_score"] * 100
        })
        
    if map_data:
        df_map = pd.DataFrame(map_data)
        df_map_grouped = df_map.groupby(["Location", "Relocate"]).agg(
            Count=("Score", "count"),
            AvgScore=("Score", "mean")
        ).reset_index()
        ...
        for _, row in df_map_grouped.iterrows():
            loc = row["Location"]
            coords = CITY_COORDS.get(loc, (20.5937, 78.9629))
            latitudes.append(coords[0])
            longitudes.append(coords[1])
            
        df_map_grouped["lat"] = latitudes
        df_map_grouped["lon"] = longitudes
        ...
        fig_map = px.scatter_geo(
            ...
            color="Relocate",
            hover_name="Location",
            hover_data={"lat": False, "lon": False, "Count": True, "AvgScore": ":.1f%"},
            color_discrete_map={"Willing to Relocate": green, "Not Willing to Relocate": red},
        ...
        fig_map.update_geos(
            fitbounds="locations",
            showcountries=True,
            countrycolor="rgba(255,255,255,0.15)" if IS_DARK else "rgba(0,0,0,0.15)",
            showland=True,
            landcolor="#0e0e11" if IS_DARK else "#f8fafc",
            showocean=True,
            oceancolor="#09090b" if IS_DARK else "#f1f5f9",
            showlakes=False,
            showrivers=False
        )
        
        map_layout = PLOT_LAYOUT.copy()
        map_layout["margin"] = dict(l=0, r=0, t=0, b=0)
        map_layout["height"] = 350
...
with tab_detail:
    if len(scored_candidates) > 0:
        ...
        # PDF Export Button
        try:
            pdf_data = generate_candidate_pdf(sc, xai)
            st.download_button(
                label="📥 Export Candidate Executive Summary (PDF)",
                data=bytes(pdf_data),
                file_name=f"Redrob_Summary_{prof['name'].replace(' ', '_')}.pdf",
        ...
        with col_det1:
            ...
            # 3. Requirements Matching Visualizer
            st.markdown(f"""
            <div class="chart-wrap">
                <div class="chart-title">🔍 Side-by-Side Requirements Matching Analysis</div>
                <div class="chart-subtitle">Direct comparison of Job Description requirements vs Candidate credentials</div>
            """, unsafe_allow_html=True)
            
            m_col1, m_col2 = st.columns(2)
            with m_col1:
                st.markdown(f"<b style='color:{text}; font-size:0.8rem;'>Job Requirements</b>", unsafe_allow_html=True)
                req_skills_html = "".join([f"<div style='font-size:0.78rem; margin:4px 0;'><span style='color:{accent};'>●</span> {s.title()} <span style='font-size:0.65rem; color:{text_muted};'>(Required)</span></div>" for s in jd_req_skills])
                pref_skills_html = "".join([f"<div style='font-size:0.78rem; margin:4px 0;'><span style='color:{green};'>●</span> {s.title()} <span style='font-size:0.65rem; color:{text_muted};'>(Preferred)</span></div>" for s in jd_pref_skills])
                st.markdown(req_skills_html + pref_skills_html, unsafe_allow_html=True)
                st.markdown(f"<div style='font-size:0.78rem; margin-top:10px;'><b>Target Experience:</b> {jd_struct['experience_min']} - {jd_struct['experience_max']} Years</div>", unsafe_allow_html=True)
                
            with m_col2:
                st.markdown(f"<b style='color:{text}; font-size:0.8rem;'>Candidate Alignment</b>", unsafe_allow_html=True)
                cand_skill_map = {s["name"].lower(): s for s in c.get("skills", [])}
                alignment_html = ""
                for s in jd_req_skills:
                    s_lower = s.lower()
                    matched = False
                    for cs in cand_skill_map:
                        if s_lower in cs or cs in s_lower:
                            matched = True
                            break
                    if matched:
                        alignment_html += f"<div style='font-size:0.78rem; margin:4px 0; color:{green};'><b>✓ Match:</b> {s.title()}</div>"
                    else:
                        alignment_html += f"<div style='font-size:0.78rem; margin:4px 0; color:{red};'><b>✗ Missing:</b> {s.title()}</div>"
                        
                for s in jd_pref_skills:
                    s_lower = s.lower()
                    matched = False
                    for cs in cand_skill_map:
                        if s_lower in cs or cs in s_lower:
                    ...
                    if matched:
                        ...
                        alignment_html += f"<div style='font-size:0.78rem; margin:4px 0; color:{text_muted};'><b>✗ Missing:</b> {s.title()}</div>"
                st.markdown(alignment_html, unsafe_allow_html=True)
                
                cand_yoe = prof['years_of_experience']
                if jd_struct['experience_min'] <= cand_yoe <= jd_struct['experience_max']:
                    exp_color = green
                    exp_status = "✓ Perfect Fit"
                else:
                    exp_color = amber
                    exp_status = f"⚠ Outside Target ({cand_yoe} Years)"
                st.markdown(f"<div style='font-size:0.78rem; margin-top:10px; color:{exp_color};'><b>Experience Fit:</b> {exp_status}</div>", unsafe_allow_html=True)
        ...
        with col_det2:
            ...
            # 4. AI Chat Section (Gemini Co-Pilot)
            st.markdown("---")
            st.subheader("💬 Chat with Candidate (AI Co-Pilot)")
            
            if "gemini_api_key" in st.session_state and st.session_state.gemini_api_key:
                import google.generativeai as genai
                genai.configure(api_key=st.session_state.gemini_api_key)
                
                chat_key = f"chat_history_{c['candidate_id']}"
                ...
                for msg in st.session_state[chat_key]:
                    ...
                    if role == "user":
                        ...
                        st.markdown(f"**AI Co-Pilot:** {content}")
                        
                user_question = st.text_input("Ask a question about this candidate's profile:", key=f"chat_input_{c['candidate_id']}")
                if user_question:
                    st.session_state[chat_key].append({"role": "user", "content": user_question})
                    
                    context = f"""
                    You are an expert AI recruiter assisting a hiring manager.
                    ...
                    Provide a concise, professional, and objective response analyzing the candidate's suitability based on the job description.
                    """
                    ...
                    try:
                        ...
                        st.error(f"Error communicating with Gemini: {chat_err}")
            else:
                st.info("💡 To chat with this candidate and get AI-driven insights, please enter your Gemini API Key in the sidebar.")
    else:
